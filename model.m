...
rho = @(T) params.rho_As(1) + params.rho_As(2).*T; %kg/m^3
...
sb = 5.670367e-8; %steffan-boltzmann constant
...
% Simulation Controls
n = 40; % number of nodes in x or y direction
m = n;
...
%     Bi_mat(n, m) = (dx * (h_top + h_side)) / (k_mat(n,m));
    ...
    Bi_top = dx*h_top./k_mat;
    Bi_side = dx*h_side./k_mat;
    Bi_bot = dx*h_bot./k_mat;
    ...
    dt(2:n-1,1) = (CFL*dx^2) ./ (2*al_mat(2:n-1,1).*(2+Bi_top(2:n-1,1)+2*Br_mat(2:n-1,1).*T(2:n-1,1).^3)); %left
    dt(2:n-1,m) = CFL*(dx^2) ./ (2*al_mat(2:n-1,m).*(2+Bi_bot(2:n-1,m)+2*Br_mat(2:n-1,m).*T(2:n-1,m).^3)); %right
    dt(1,2:m-1) = CFL*(dx^2) ./ (2*al_mat(1,2:n-1).*(2+Bi_side(1,2:n-1)+2*Br_mat(1,2:m-1).*T(1,2:n-1).^3)); %top
    dt(n,2:m-1) = CFL*(dx^2) ./ (2*al_mat(n,2:n-1).*(2+Bi_side(n,2:n-1)+2*Br_mat(n,2:m-1).*T(n,2:n-1).^3)); %bot
    
    % CORNERS:
    dt(1,1) = CFL*(dx^2) /...
        ((2*al_mat(1,1)) * (2 + Bi_top(1,1)+ Bi_side(1,1) + 2*Br_mat(1,1)*T(1,1)^3));
    dt(1,m) = CFL*(dx^2) /...
        ((2*al_mat(1,m)) * (2 + Bi_top(1,m)+ Bi_side(1,m) + 2*Br_mat(1,m)*T(1,m)^3));
    dt(n,1) = CFL*(dx^2) /...
        ((2*al_mat(n,1)) * (2 + Bi_bot(n,1)+ Bi_side(n,1) + 2*Br_mat(n,1)*T(n,1)^3));
    dt(n,m) = CFL*(dx^2) /...
        ((2*al_mat(n,m)) * (2 + Bi_bot(n,m)+ Bi_side(n,m) + 2*Br_mat(n,m)*T(n,m)^3));
...
%        b(i,n) = 2*Fo_mat(n,i) .* (Br_mat(n,i) * T_inf^3 + Bi_mat(n,i))*T_inf;  
       ...
       b(1,i) = 2*Fo_mat(1,i).*Bi_top(1,i)*T_inf +...
           2*Fo_mat(1,i).*Br_mat(1,i)*(T_inf^4 -T(1,i)^4);  
       
       b(n,i) = 2*Fo_mat(n,i).*Bi_bot(n,i)*T_inf +...
           2*Fo_mat(n,i).*Br_mat(n,i)*(T_inf^4 -T(n,i)^4);  
       
       
       b(i,1) = 2*Fo_mat(i,1).*Bi_side(i,1)*T_inf +...
           2*Fo_mat(i,1).*Br_mat(i,1)*(T_inf^4 -T(i,1)^4);  
       
       b(i,n) = 2*Fo_mat(i,n).*Bi_side(i,n)*T_inf +...
...
%     
    b(1,1) = 2*Fo_mat(1,1) *(Bi_top(1,1)+Bi_side(1,1)) * T_inf+...
           4*Fo_mat(1,1).*Br_mat(1,1)*(T_inf^4 -T(1,1)^4);
       
    b(1,n) = 2*Fo_mat(1,n)*(Bi_top(1,n)+Bi_side(1,n))  * T_inf+...
           4*Fo_mat(1,n).*Br_mat(1,n)*(T_inf^4 -T(1,n)^4);
       
    b(n,1) = 2*Fo_mat(n,1) *(Bi_bot(n,1)+Bi_side(n,1))  * T_inf+...
           4*Fo_mat(n,1).*Br_mat(n,1)*(T_inf^4 -T(n,1)^4);
    
    b(n,n) = 2*Fo_mat(n,n) *(Bi_bot(n,n)+Bi_side(n,n))  * T_inf+...
    ...
    while (err > err_max)
        ...
        A_ii = 1+2*Fo_mat(1,1)*(2+Bi_side(1,1)+Bi_top(1,1));
        ...
        for j = 2:n-1
            ...
            A_ii = 1+2*Fo_mat(1,j)*(2+Bi_top(1,j));
        ...
        A_ii = 1+2*Fo_mat(1,m)*(2+Bi_side(1,m)+Bi_top(1,m));
        ...
        for i = 2:n-1
            ...
            A_ii = 1+2*Fo_mat(i,1)*(2+Bi_side(i,1));
        ...
        for i =  2:n-1
            ...
            A_ii = 1+2*Fo_mat(i,m)*(2+Bi_side(i,m));
        ...
        A_ii = 1+2*Fo_mat(n,1)*(2+Bi_side(n,1)+Bi_bot(n,1));
        ...
        for j = 2:n-1
            ...
            A_ii = 1+2*Fo_mat(n,j)*(2+Bi_bot(n,j));
        ...
        A_ii = 1+2*Fo_mat(n,m)*(2+Bi_bot(n,m)+Bi_side(n,m));
...
figure(1)
subplot(1,3,1)
plot(data_time,data_temp(:,1),'kx',...
    save_time,save_data_mat(:,1),'r')

figure(1)
subplot(1,3,2)
plot(data_time,data_temp(:,2),'kx',...
    save_time,save_data_mat(:,2),'r')

subplot(1,3,3)
plot(data_time,data_temp(:,3),'kx',...
    save_time,save_data_mat(:,3),'r')
